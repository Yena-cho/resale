package kr.co.finger.damoa.scheduler.model;

/**
 * @author  by puki
 * @date    2018. 5. 13.
 * @desc    최초생성
 * @version 
 * 
 */
public class LayOut {

	private String header;		// 헤더
	private String dummy;		// dummy
	private String reqDate;		// 의뢰날짜

	private String bHeader; 	//  헤더
	private String seq;			//  seq
	private String chaCd;		//  코드
	private String notiMascd;
	private String msgTitle;	//	메시지 타이틀
	private String recName;		//	수신자명
	private String msg;
	private String msg1;		//	메시지1
	private String msg2;		//	메시지2
	private String msg3;		//	메시지3
	private String msg4;		//	메시지4
	private String msg5;		//	메시지5
	private String msg6;		//	메시지6
	private String msg7;		//	메시지7
	private String msg8;		//	메시지8
	private String msg9;		//	메시지9
	private String msg10;		//	메시지10
	private String offName;		//	학원명
	private String offTel;		//	학원전화번호
	private String vano;		//	계좌번호
	private String payYear;		//	납기년
	private String payMonth;	//	납기월
	private String payDay;		//	납기일
	private String grpName;		//	그룹명
	private String item1;		//	항목명1
	private String amt1;		//	금액1
	private String etc1;		//	비고1
	private String item2;		//	항목명2
	private String amt2;		//	금액2
	private String etc2;		//	비고2
	private String item3;		//	항목명3
	private String amt3;		//	금액3
	private String etc3;		//	비고3
	private String item4;		//	항목명4
	private String amt4;		//	금액4
	private String etc4;		//	비고4
	private String item5;		//	항목명5
	private String amt5;		//	금액5
	private String etc5;		//	비고5
	private String item6;		//	항목명6
	private String amt6;		//	금액6
	private String etc6;		//	비고6
	private String item7;		//	항목명7
	private String amt7;		//	금액7
	private String etc7;		//	비고7
	private String item8;		//	항목명8
	private String amt8;		//	금액8
	private String etc8;		//	비고8
	private String item9;		//	항목명9
	private String amt9;		//	금액9
	private String etc9;		//	비고9
	private String item10;		//	항목명10
	private String amt10;		//	금액10
	private String etc10;		//	비고10
	private String zipCd;		//	우편번호
	private String addr;		//	주소
	private String offName2;	//	학원명 
	private String hp;			//	핸드폰번호
	private String addMsg;		//	추가메시지
	private String tHeader;		//  헤더
	private String billGubn;
	private String masMonth;
	private String ownerTel;
	private String payItemAmt;
	private String addr2;
	private String sort1Cd;
	private String sort2Cd;
	
	public String getMasMonth() {
		return masMonth;
	}
	public void setMasMonth(String masMonth) {
		this.masMonth = masMonth;
	}
	public String getBillGubn() {
		return billGubn;
	}
	public void setBillGubn(String billGubn) {
		this.billGubn = billGubn;
	}
	private String idx;
	
	public String getHeader() {
		return header;
	}
	public void setHeader(String header) {
		this.header = header;
	}
	public String getDummy() {
		return dummy;
	}
	public void setDummy(String dummy) {
		this.dummy = dummy;
	}
	public String getReqDate() {
		return reqDate;
	}
	public void setReqDate(String reqDate) {
		this.reqDate = reqDate;
	}
	public String getbHeader() {
		return bHeader;
	}
	public void setbHeader(String bHeader) {
		this.bHeader = bHeader;
	}
	public String getSeq() {
		return seq;
	}
	public void setSeq(String seq) {
		this.seq = seq;
	}
	public String getChaCd() {
		return chaCd;
	}
	public void setChaCd(String chaCd) {
		this.chaCd = chaCd;
	}
	public String getNotiMascd() {
		return notiMascd;
	}
	public void setNotiMascd(String notiMascd) {
		this.notiMascd = notiMascd;
	}
	public String getMsgTitle() {
		return msgTitle;
	}
	public void setMsgTitle(String msgTitle) {
		this.msgTitle = msgTitle;
	}
	public String getRecName() {
		return recName;
	}
	public void setRecName(String recName) {
		this.recName = recName;
	}
	public String getMsg() {
		return msg;
	}
	public void setMsg(String msg) {
		this.msg = msg;
	}
	public String getMsg1() {
		return msg1;
	}
	public void setMsg1(String msg1) {
		this.msg1 = msg1;
	}
	public String getMsg2() {
		return msg2;
	}
	public void setMsg2(String msg2) {
		this.msg2 = msg2;
	}
	public String getMsg3() {
		return msg3;
	}
	public void setMsg3(String msg3) {
		this.msg3 = msg3;
	}
	public String getMsg4() {
		return msg4;
	}
	public void setMsg4(String msg4) {
		this.msg4 = msg4;
	}
	public String getMsg5() {
		return msg5;
	}
	public void setMsg5(String msg5) {
		this.msg5 = msg5;
	}
	public String getMsg6() {
		return msg6;
	}
	public void setMsg6(String msg6) {
		this.msg6 = msg6;
	}
	public String getMsg7() {
		return msg7;
	}
	public void setMsg7(String msg7) {
		this.msg7 = msg7;
	}
	public String getMsg8() {
		return msg8;
	}
	public void setMsg8(String msg8) {
		this.msg8 = msg8;
	}
	public String getMsg9() {
		return msg9;
	}
	public void setMsg9(String msg9) {
		this.msg9 = msg9;
	}
	public String getMsg10() {
		return msg10;
	}
	public void setMsg10(String msg10) {
		this.msg10 = msg10;
	}
	public String getOffName() {
		return offName;
	}
	public void setOffName(String offName) {
		this.offName = offName;
	}
	public String getOffTel() {
		return offTel;
	}
	public void setOffTel(String offTel) {
		this.offTel = offTel;
	}
	public String getVano() {
		return vano;
	}
	public void setVano(String vano) {
		this.vano = vano;
	}
	public String getPayYear() {
		return payYear;
	}
	public void setPayYear(String payYear) {
		this.payYear = payYear;
	}
	public String getPayMonth() {
		return payMonth;
	}
	public void setPayMonth(String payMonth) {
		this.payMonth = payMonth;
	}
	public String getPayDay() {
		return payDay;
	}
	public void setPayDay(String payDay) {
		this.payDay = payDay;
	}
	public String getGrpName() {
		return grpName;
	}
	public void setGrpName(String grpName) {
		this.grpName = grpName;
	}
	public String getItem1() {
		return item1;
	}
	public void setItem1(String item1) {
		this.item1 = item1;
	}
	public String getAmt1() {
		return amt1;
	}
	public void setAmt1(String amt1) {
		this.amt1 = amt1;
	}
	public String getEtc1() {
		return etc1;
	}
	public void setEtc1(String etc1) {
		this.etc1 = etc1;
	}
	public String getItem2() {
		return item2;
	}
	public void setItem2(String item2) {
		this.item2 = item2;
	}
	public String getAmt2() {
		return amt2;
	}
	public void setAmt2(String amt2) {
		this.amt2 = amt2;
	}
	public String getEtc2() {
		return etc2;
	}
	public void setEtc2(String etc2) {
		this.etc2 = etc2;
	}
	public String getItem3() {
		return item3;
	}
	public void setItem3(String item3) {
		this.item3 = item3;
	}
	public String getAmt3() {
		return amt3;
	}
	public void setAmt3(String amt3) {
		this.amt3 = amt3;
	}
	public String getEtc3() {
		return etc3;
	}
	public void setEtc3(String etc3) {
		this.etc3 = etc3;
	}
	public String getItem4() {
		return item4;
	}
	public void setItem4(String item4) {
		this.item4 = item4;
	}
	public String getAmt4() {
		return amt4;
	}
	public void setAmt4(String amt4) {
		this.amt4 = amt4;
	}
	public String getEtc4() {
		return etc4;
	}
	public void setEtc4(String etc4) {
		this.etc4 = etc4;
	}
	public String getItem5() {
		return item5;
	}
	public void setItem5(String item5) {
		this.item5 = item5;
	}
	public String getAmt5() {
		return amt5;
	}
	public void setAmt5(String amt5) {
		this.amt5 = amt5;
	}
	public String getEtc5() {
		return etc5;
	}
	public void setEtc5(String etc5) {
		this.etc5 = etc5;
	}
	public String getItem6() {
		return item6;
	}
	public void setItem6(String item6) {
		this.item6 = item6;
	}
	public String getAmt6() {
		return amt6;
	}
	public void setAmt6(String amt6) {
		this.amt6 = amt6;
	}
	public String getEtc6() {
		return etc6;
	}
	public void setEtc6(String etc6) {
		this.etc6 = etc6;
	}
	public String getItem7() {
		return item7;
	}
	public void setItem7(String item7) {
		this.item7 = item7;
	}
	public String getAmt7() {
		return amt7;
	}
	public void setAmt7(String amt7) {
		this.amt7 = amt7;
	}
	public String getEtc7() {
		return etc7;
	}
	public void setEtc7(String etc7) {
		this.etc7 = etc7;
	}
	public String getItem8() {
		return item8;
	}
	public void setItem8(String item8) {
		this.item8 = item8;
	}
	public String getAmt8() {
		return amt8;
	}
	public void setAmt8(String amt8) {
		this.amt8 = amt8;
	}
	public String getEtc8() {
		return etc8;
	}
	public void setEtc8(String etc8) {
		this.etc8 = etc8;
	}
	public String getItem9() {
		return item9;
	}
	public void setItem9(String item9) {
		this.item9 = item9;
	}
	public String getAmt9() {
		return amt9;
	}
	public void setAmt9(String amt9) {
		this.amt9 = amt9;
	}
	public String getEtc9() {
		return etc9;
	}
	public void setEtc9(String etc9) {
		this.etc9 = etc9;
	}
	public String getItem10() {
		return item10;
	}
	public void setItem10(String item10) {
		this.item10 = item10;
	}
	public String getAmt10() {
		return amt10;
	}
	public void setAmt10(String amt10) {
		this.amt10 = amt10;
	}
	public String getEtc10() {
		return etc10;
	}
	public void setEtc10(String etc10) {
		this.etc10 = etc10;
	}
	public String getZipCd() {
		return zipCd;
	}
	public void setZipCd(String zipCd) {
		this.zipCd = zipCd;
	}
	public String getAddr() {
		return addr;
	}
	public void setAddr(String addr) {
		this.addr = addr;
	}
	public String getOffName2() {
		return offName2;
	}
	public void setOffName2(String offName2) {
		this.offName2 = offName2;
	}
	public String getHp() {
		return hp;
	}
	public void setHp(String hp) {
		this.hp = hp;
	}
	public String getAddMsg() {
		return addMsg;
	}
	public void setAddMsg(String addMsg) {
		this.addMsg = addMsg;
	}
	public String getIdx() {
		return idx;
	}
	public void setIdx(String idx) {
		this.idx = idx;
	}
	public String gettHeader() {
		return tHeader;
	}
	public void settHeader(String tHeader) {
		this.tHeader = tHeader;
	}
	public String getOwnerTel() {
		return ownerTel;
	}
	public void setOwnerTel(String ownerTel) {
		this.ownerTel = ownerTel;
	}
	public String getPayItemAmt() {
		return payItemAmt;
	}
	public void setPayItemAmt(String payItemAmt) {
		this.payItemAmt = payItemAmt;
	}
	public String getAddr2() {
		return addr2;
	}
	public void setAddr2(String addr2) {
		this.addr2 = addr2;
	}

	public String getSort1Cd() {
		return sort1Cd;
	}

	public void setSort1Cd(String sort1Cd) {
		this.sort1Cd = sort1Cd;
	}

	public String getSort2Cd() {
		return sort2Cd;
	}

	public void setSort2Cd(String sort2Cd) {
		this.sort2Cd = sort2Cd;
	}
}

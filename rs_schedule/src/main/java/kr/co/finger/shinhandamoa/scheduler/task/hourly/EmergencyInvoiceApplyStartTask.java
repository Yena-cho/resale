 ...
 */
@Component
public class EmergencyInvoiceApplyStartTask {
    private static final Logger LOGGER = LoggerFactory.getLogger(EmergencyInvoiceApplyStartTask.class);

    @Autowired
    private InvoiceService invoiceService;
//TODO
//    @Scheduled(cron = "${emergency-invoice-apply-start-task.schedule.cron}")
    public void start() {
        run();
    }

    protected void run() {
        LOGGER.debug("시작 : 고지서 출력 의뢰 (퀵) 시작 작업 ");

        final InvoiceApplySF invoiceApplySF = new InvoiceApplySF();
        invoiceApplySF.setDeliveryTypeCd(InvoiceApplyDeliveryType.EMERGENCY);
        ...
        LOGGER.debug("종료 : 고지서 출력 의뢰 (퀵) 시작 작업 ");
    }
}

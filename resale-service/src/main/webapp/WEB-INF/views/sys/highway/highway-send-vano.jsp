...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">
                <strong>PG 가상계좌 전송및 리스트</strong>
            </li>
        </ol>
        <p class="page-description">PG에서 요청된 가상계좌를 전송하는 화면입니다.  </p>
    </div>
    <div class="col-lg-2">
                    ...
                    <div class="ibox-title">
                        ...
                        <div class="col-lg-6 form-inline form-searchOrderBy">
                            <select class="form-control" name="nSearchOrderBy" id="orderBy" onchange="pageChange();">
                                <option value="clientId">기관코드 내림차순</option>
...
<script>
    ...
    function fnGrid(result, obj) {
        ...
        $.each(result.resultList, function (i, v) {
            ...
            str += '<td>' + v.assign_dt + '</td>'; //가상계좌할당일
            str += '<td>' + v.vano_count + '</td>'; //가상계좌채번수
            str += '<td>' + v.remark + '</td>'; //remark
            str += '<td>' + v.send_dt + '</td>'; //전송일
            str += '<td>' + v.result_msg + '</td>'; //전송메시지
            ...
            if(v.checkyn == "N"){
                str += '<td>'
                str += '<button type="button" class="btn btn-xs btn-success" onclick=sendVano(\'' + v.trans_vano_hist_id + '\')>전송</button>';
                str += '</td>';
            }else if(v.checkyn == "F") {
                str += '<td>'
                str += '<button type="button" class="btn btn-xs btn-success" onclick=sendVano(\'' + v.trans_vano_hist_id + '\')>재전송</button>';
                str += '</td>';
            }
            str += '</tr>';
        });
    ...
    function sendVano(TRANS_VANO_HIST_ID){



        var param = {
            trnVanoId : TRANS_VANO_HIST_ID
        };
        ...
        $.ajax({
            type: "post",
            async: true,
            url: "/sys/highway/vanoSend",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(param),
            success: function (response) {
                    swal({
                        type:'success',
                        text:'가상계좌가 고속도로로 전송 요청 되었습니다. 최대 5분 소요됩니다.',
                        confirmButtonColor: '#3085d6',

...
<div class="row wrapper border-bottom white-bg page-heading">
  <div class="col-lg-10">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">
        <strong>PG 기관정보 변경페이지</strong>
      </li>
    </ol>
    <p class="page-description">PG관리자에서 보낸 기관의 상태를 변경하는 페이지입니다.  </p>
  </div>
  <div class="col-lg-2">
...
<script>
  ...
  function fnSearch(page) {
    ...
    $.ajax({
      type : "post",
      async : true,
      url: "/sys/chaMgmt/pgChaUpdateList",
      contentType : "application/json; charset=utf-8",
      data : JSON.stringify(param),
  ...
  function fnGrid(result, obj) {
    ...
    $.each(result.resultList, function (i, v) {
      ...
      str += '<td>' + v.pullDt + '</td>'; //변경요청일
      str += '<td>' + v.limitDay + '</td>'; //변경일입금금액
      str += '<td>' + v.limitDayCnt + '</td>'; //변경일제한횟수
      str += '<td>' + v.limitOnce + '</td>'; //변경1회입금가능금액
      str += '<td>' + v.assignDt + '</td>'; //변경적용일
      if(v.amtchkty == "Y"){//변경방식
        str += '<td>승인방식</td>';
      }else if(v.amtchkty == "N"){
        str += '<td>통지방식</td>';
      }else if(v.amtchkty == "L"){
        str += '<td>한도방식</td>';
      }
      if(v.assignYn == "R"){//적용상태
        str += '<td>변경대기</td>';
      }else if(v.assignYn == "Y"){
        str += '<td>변경완료</td>';
      }else if(v.assignYn == "N"){
        str += '<td>변경만료</td>';
      }

      if(v.assignYn == "R"){//적용버튼
        str += '<td>'
        str += '<button type="button" class="btn btn-xs btn-success" onclick=pgChaUpdateAjax(\'' + v.pgPullSeq + '\')>적용</button>';
        str += '</td>';
      }
      str += '</tr>';
    });
  ...
  function pgChaUpdateAjax(pgPullSeq){



    var param = {
      pgPullSeq : pgPullSeq //가상계좌 전송 고유번호
    };
    ...
    $.ajax({
      type: "post",
      async: true,
      url: "/sys/chaMgmt/pgChaUpdateAjax",
      contentType: "application/json; charset=utf-8",
      data: JSON.stringify(param),
      success: function (response) {
        swal({
          type:'success',
          text:'변경요청이 정상처리 되었습니다.',
          confirmButtonColor: '#3085d6',

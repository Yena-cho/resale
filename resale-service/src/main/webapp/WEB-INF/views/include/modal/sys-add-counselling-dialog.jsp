...
<script>
    function addCust() {
        var mday1 = replaceDot($("#mday1").val());
        var mdata3 = '';

        if ($("#modal-sys-add-counselling-dialog #mdata2 option:selected").val() == 01) {
            mdata3 = "사이트이용";
        } else if ($("#modal-sys-add-counselling-dialog #mdata2 option:selected").val() == 02) {
            mdata3 = "증명서 발급";
        } else if ($("#modal-sys-add-counselling-dialog #mdata2 option:selected").val() == 03) {
            mdata3 = "결제";
        } else if ($("#modal-sys-add-counselling-dialog #mdata2 option:selected").val() == 04) {
            mdata3 = "오류 및 불편사항";
        } else if ($("#modal-sys-add-counselling-dialog #mdata2 option:selected").val() == 05) {
            mdata3 = "기타";
        } else {
            mdata3 = "고지/수납";
        }

        var mdata1 = $("#modal-sys-add-counselling-dialog #mdata1").val();
        if (mdata1.length == 11) {
            var kw = mdata1.substring(0, 3) + "-" + mdata1.substring(3, 7) + "-" + mdata1.substring(7, 11);
            mdata1 = kw;
        }

        var emplo = $("#modal-sys-add-counselling-dialog #mdata5").val();
        var mtitle = $("#modal-sys-add-counselling-dialog #mtitle").val();
        var mcontents = $("#modal-sys-add-counselling-dialog #mcontents").val();
        if (emplo == null || emplo == "") {
            swal({
                type: 'info',
                text: '상담직원명을 입력해 주세요',
        ...
        } else if ($("#modal-sys-add-counselling-dialog select#mwho").val() === "1" &&
            ($("#mCode").val() == null || $("#mCode").val() == "")) {
            swal({
                type: 'info',
                text: '기관코드를 입력해 주세요',
                confirmButtonColor: '#3085d6',
        ...
        } else if ($("#mwriter").val() == null || $("#mwriter").val() == "") {
            swal({
                type: 'info',
                text: '신청자명을 입력해 주세요',
                confirmButtonColor: '#3085d6',
        ...
        } else if ($("#mdata1").val() == null || $("#mdata1").val() == "") {
            swal({
                type: 'info',
            ...
            var param = {
                data2: $("#modal-sys-add-counselling-dialog #mdata2 option:selected").val(),            // 예약사유코드
                data3: mdata3,                                                                          // 예약사유명
                data5: emplo,                                                                           // 상담직원
                chaCd: $("#modal-sys-add-counselling-dialog #mCode").val(),                             // 기관코드
                data4: $("#modal-sys-add-counselling-dialog #mstatus option:selected").val(),           // 상태
                writer: $("#modal-sys-add-counselling-dialog #mwriter").val(),                          // 신청자명
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.retCode == "0000") {
                        ...
                        $("#modal-sys-add-counselling-dialog").modal("hide");
                        search();
                    } else {
                        swal({
                            type: 'success',
...
<div class="inmodal modal fade" id="modal-sys-add-counselling-dialog" tabindex="-1" role="dialog" aria-labelledby="regPayer"
     aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="mdtitle">전화상담 진행상황 등록</h4>
            </div>
                ...
                <form>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col col-md-6 col-12">
                            ...
                            <div class="date">
                                <input type="text" class="form-control" value="" id="mday1">
                            </div>
                        </div>
                    </div>
                    <div class="row form-group">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="addCust()" id="msavebtn">저장</button>
            </div>
        </div>
    </div>
</div>
	...
	var elBucketName = document.getElementById("bucketName").value;
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/noticeMultiplePhotoUpload'; 	//upload URL
    	...
    	for(var j=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
					...
					sleep(100);
	    			callAjaxForHTML5(tempFile,sUploadURL, elBucketName);
	    		}
	    	}catch(e){}
    		tempFile = null;
    	}
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL, elBucketName){
        ...
        var url = sUploadURL;
    	// document.getElementById("ufile").files[0] = tempFile;
        // formData.encrypt = "multipart/form-data";
        formData.append("content-type","multipart/form-data");
    	// formData.append("ufile", document.getElementById("ufile").files[0]);
        ...
        formData.append("bucket", elBucketName);
    	// formData.append("enctype", "multipart/form-data");
    	// formData.append("method", "post");
    	// formData.append("action", sUploadURL);
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
	    			...
	    			if(aSubTemp[2] != "" && aSubTemp[2] != undefined && aSubTemp[2] != null){
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/noticePhotoUpload?BNM='+elBucketName,	//샘플 URL입니다.
 	        sCallback : '/assets/smarteditor2/workspace/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소

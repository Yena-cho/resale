...
public class ExcelSaveNotice extends AbstractExcelView2 {
	...
	protected void buildExcelDocument(Map<String, Object> model, XSSFWorkbook workbook, HttpServletRequest request,
        ...
        for(int i = 0; i < idx; i++) {
        	...
        	hRow.createCell(i++).setCellValue("첨부파일");
        ...
        for(SysDTO dto : listExcel) {
        	...
        	if(dto.getBbs() == 6) {
        		row.createCell(i++).setCellValue("납부자");
        	}else if(dto.getBbs() == 7) {
        		row.createCell(i++).setCellValue("로그인전");
        	}else {
        		row.createCell(i++).setCellValue("기관담당자");
			}
        	row.createCell(i++).setCellValue(dto.getFilename());

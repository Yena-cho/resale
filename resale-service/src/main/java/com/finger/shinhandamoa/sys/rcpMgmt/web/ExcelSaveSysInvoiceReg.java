...
public class ExcelSaveSysInvoiceReg extends AbstractExcelView2 {
	...
	protected void buildExcelDocument(Map<String, Object> model, XSSFWorkbook workbook, HttpServletRequest request,
		...
		List<SysInvoiceDTO> listExcel = (List<SysInvoiceDTO>)model.get("list"); // 목록
        ...
        for(int i = 0; i < idx; i++) {
        	hRow.createCell(i++).setCellValue("사용월");
        	hRow.createCell(i++).setCellValue("기관코드");
    		hRow.createCell(i++).setCellValue("기관명");
        	hRow.createCell(i++).setCellValue("공급가액");
        	hRow.createCell(i++).setCellValue("부가세");
        	hRow.createCell(i++).setCellValue("이메일주소");
        	hRow.createCell(i++).setCellValue("발행상태");
        	hRow.createCell(i++).setCellValue("승인번호");
        	hRow.createCell(i++).setCellValue("발행일자");
        }
        
        int rowIndex = 1;
        
        for(SysInvoiceDTO dto : listExcel) {
        	int i = 0;

        	row = worksheet.createRow(rowIndex);
			
        	row.createCell(i++).setCellValue(dto.getUseDay());
    		row.createCell(i++).setCellValue(dto.getComCd());
    		row.createCell(i++).setCellValue(dto.getComNm());
    		row.createCell(i++).setCellValue(dto.getPrice());
    		row.createCell(i++).setCellValue(dto.getTax());
    		row.createCell(i++).setCellValue(dto.getEmail());
    		if(dto.getIssueSt().equals("1")) {
    			row.createCell(i++).setCellValue("발행");
    		}else {
    			row.createCell(i++).setCellValue("미발행");
    		}
    		row.createCell(i++).setCellValue(dto.getIssueNm());
    		row.createCell(i++).setCellValue(dto.getIssueDay());
    		rowIndex++;
        }

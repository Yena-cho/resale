...
public class ExcelFailDownload extends AbstractExcelView2 {
    ...
    protected void buildExcelDocument(Map<String, Object> model, XSSFWorkbook workbook, HttpServletRequest request,
        ...
        String excelName = sCurTime + "_" + chaCd + "_청구실패목록.xlsx";
        Sheet worksheet = null;
        Row TophRow = null;
        Row hRow = null;
        Row row = null;

        List<ClaimDTO> list = (List<ClaimDTO>) model.get("list");
        ...
        if (cusGbCnt == 1) {
            TophRow.createCell(2).setCellValue(cdList.get(0).getCodeName());
        }
        if (cusGbCnt == 2) {
            TophRow.createCell(2).setCellValue(cdList.get(0).getCodeName());
            TophRow.createCell(3).setCellValue(cdList.get(1).getCodeName());
        }
        if (cusGbCnt == 3) {
            TophRow.createCell(2).setCellValue(cdList.get(0).getCodeName());
            TophRow.createCell(3).setCellValue(cdList.get(1).getCodeName());
            TophRow.createCell(4).setCellValue(cdList.get(2).getCodeName());
        }
        if (cusGbCnt == 4) {
            TophRow.createCell(2).setCellValue(cdList.get(0).getCodeName());
            TophRow.createCell(3).setCellValue(cdList.get(1).getCodeName());
            TophRow.createCell(4).setCellValue(cdList.get(2).getCodeName());
            TophRow.createCell(5).setCellValue(cdList.get(3).getCodeName());
        }
        ...
        for (ClaimDTO dto : list) {
            ...
            row.createCell(1).setCellValue(dto.getCusName());

            if (cusGbCnt == 1) {
                row.createCell(2).setCellValue(dto.getCusGubn1());
            }
            if (cusGbCnt == 2) {
                row.createCell(2).setCellValue(dto.getCusGubn1());
                row.createCell(3).setCellValue(dto.getCusGubn2());
            }
            if (cusGbCnt == 3) {
                row.createCell(2).setCellValue(dto.getCusGubn1());
                row.createCell(3).setCellValue(dto.getCusGubn2());
                row.createCell(4).setCellValue(dto.getCusGubn3());
            }
            if (cusGbCnt == 4) {
                row.createCell(2).setCellValue(dto.getCusGubn1());
                row.createCell(3).setCellValue(dto.getCusGubn2());
                row.createCell(4).setCellValue(dto.getCusGubn3());
                row.createCell(5).setCellValue(dto.getCusGubn4());
            }
            ...
            for (int j = 0; j < iListCnt; j++) {
                for (ClaimItemDTO each : dto.getDetailsList()) {
                    ...
                    row.createCell(dCnt + (j * 2) + 1).setCellValue(each.getPtrItemRemark());
                    break;
                }
            }

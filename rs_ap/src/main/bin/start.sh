...
if [ -z "${PID}" ]; then
    ...
    nohup $RUN_JAVA -DDamoaMessageServer $JAVA_OPTS -Dlogging.config=file:$MESSAGE_SERVER_HOME/config/logback.xml -Dspring.config.location=$MESSAGE_SERVER_HOME/config/ -jar $MESSAGE_SERVER_HOME/lib/damoa-shinhan-agent-0.0.1-SNAPSHOT.jar 1>/dev/null 2>&1 &
    echo $! > ${PID_FILE}
else

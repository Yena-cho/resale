...
public class MessageFormatConverter implements Converter {
    ...
    public Object unmarshal(HierarchicalStreamReader hsr, UnmarshallingContext uc) {
        ...
        if (!StringUtils.isEmpty(hsr.getAttribute("tranTypeCode"))) {
            message.setTranTypeCode(hsr.getAttribute("tranTypeCode"));
        }
        if (!StringUtils.isEmpty(hsr.getAttribute("recieveVoType"))) {
            message.setRecieveVoType(hsr.getAttribute("recieveVoType"));
        }
        if (!StringUtils.isEmpty(hsr.getAttribute("desc"))) {
        ...
        while ("field".equals(hsr.getNodeName())) {
            if ("field".equals(hsr.getNodeName())) {
                ...
                if (StringUtils.isEmpty(hsr.getAttribute("ref"))) {
                    ...
                    field.setReqChk(hsr.getAttribute("reqChk"));
                    field.setResChk(hsr.getAttribute("resChk"));

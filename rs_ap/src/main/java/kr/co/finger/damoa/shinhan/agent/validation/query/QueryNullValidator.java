 ...
 */
public class QueryNullValidator implements Validator {
    private static Logger LOGGER = LoggerFactory.getLogger(QueryNullValidator.class);
    
    @Override
    public ValidationResult validate(List<NotiMas> notiMasList, ChacdInfo chacdInfo, MsgIF msgIF) {
        final QueryMessage queryMessage = (QueryMessage) msgIF;
        
        if (notiMasList == null || notiMasList.size()==0) {
            LOGGER.error("가상계좌번호[" + queryMessage.getDepositAccountNo().trim() + "] 에 해당하는 청구가 없음.");

...
public class ApplyOracleDB extends ApplyDB{
	private DBConn conn = null;

	@SuppressWarnings("rawtypes")
	private Vector vt = null;
	
	public ApplyOracleDB(){
	...
	public MirRecord selectPayData() throws UserException, Exception{
    	...
    	String qry	= "\n SELECT * "
        	...
        	+ "\n                  rcpprtendtm, "
        	+ "\n                  cusnm, "
        	+ "\n                  nvl(smsyn, 'N') as smsyn, "
        	+ "\n                  mobileno, "
        	+ "\n                  nvl(emailyn, 'N') as emailyn, "
        	+ "\n                  email, "
        	+ "\n                  content1, "
        	+ "\n                  content2, "
        	+ "\n                  content3, "
        	...
        	+ "\n                  payitemnm, "
        	+ "\n                  payitemamt, "
        	+ "\n                  nvl(payitemyn, 'N') as payitemyn, "
        	+ "\n                  nvl(rcpitemyn, 'N') as rcpitemyn, "
        	+ "\n                  rcpbusinessno, "
        	+ "\n                  rcppersonno, "
        	+ "\n                  prtpayitemnm, "
        	+ "\n                  prtcontent1, "
        	...
        	+ "\n         WHERE    transt = '01' "
        	+ "\n         ORDER BY regseq) "
        	+ "\n WHERE  ROWNUM <= " + ClientInfo.DB_ROWCNT
//        	+ "\n WHERE  ROWNUM <= 300"
        	;
	...
	public boolean InsertRcpData(AegisRMsg rMsg) throws UserException, Exception{
		...
		if("DAMOA".equalsIgnoreCase(ClientInfo.SERVICE_NAME)){
    		qry = "\n INSERT INTO " + ClientInfo.DB_TABLE + " ( "
            	...
            	+ "\n     ?, ?,  "
    			+ "\n     ?, ?, ?, ?,  "
    			+ "\n     sysdate, ?, aegistransmsg_seq.NEXTVAL "
        		+ "\n    ) ";
